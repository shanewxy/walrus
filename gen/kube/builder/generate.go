...
func Generate(c Config) error {
	...
	// Execute crd-gen/apireg-gen/webhook-gen.
	...
	// Execute openapi-gen.
	{
		...
		openapiGenArgs := openapigenargs.Args{
			OutputDir:      filepath.Join(c.ProjectDir, "pkg", "apis"),
			OutputPkg:      path.Join(c.Project, "pkg", "apis"),
		...
		if err := gengov2.Execute(
			openapigen.NameSystems(),
			openapigen.DefaultNameSystem(),
			func(g *generatorv2.Context) []generatorv2.Target {
				return openapigen.GetTargets(g, &openapiGenArgs)
			},
			gengov2.StdBuildTag,
			append(slices.Clone(internalAPIs), c.MachineryAPIs...)); err != nil {
	...
	applyConfigPkg := path.Join(c.Project, "pkg", "clients", "applyconfiguration")
	{
		...
		applyConfigGen := args.GeneratorArgs{
			InputDirs:         slices.Clone(allAPIs),
			OutputBase:        c.ProjectDir,
