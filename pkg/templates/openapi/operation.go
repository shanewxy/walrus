...
// IntersectSchema generates a openapi3.OpenAPISchema with properties intersection of s1 and s2.
...
// UnionSchema generates a openapi3.OpenAPISchema union of s1 and s2, for the same key s2 will overwrite s1's.
...
func MustSchemaEqual(s1, s2 *openapi3.Schema) bool {
	...
	if err != nil {
		klog.Warning("failed to compare schema, %w", err)
	}
