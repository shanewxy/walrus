...
func (r *ecsInstance) Log(ctx context.Context, name string, opts resourcehandler.LogOptions) error {
	for {
		select {
		case <-ctx.Done():
			return nil
		...
		if resp.ConsoleOutput != "" {
			...
			content, err := stringx.DecodeBase64(resp.ConsoleOutput)
			if err != nil {
				return err
			}
			...
			r.lastContent = content[len(content)-20:]
		}
