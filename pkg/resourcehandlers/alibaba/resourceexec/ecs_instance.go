...
func init() {
	logger := logrus.New()
	logger.SetOutput(klog.NewKlogr())
...
func (r *ecsInstance) Exec(ctx context.Context, name string, opts resourcehandler.ExecOptions) error {
	...
	defer func() {
		...
		if err := r.assistCli.SendCloseMessage(); err != nil {
			klog.Warningf("error send close message: %v", err)
		}
	}()
	...
	eg := gopool.GroupWithContextIn(ctx)
...
func (r *ecsInstance) setTerminalSize(ctx context.Context, opts resourcehandler.ExecOptions) error {
