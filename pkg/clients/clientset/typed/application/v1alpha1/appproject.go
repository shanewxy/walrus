...
type AppProjectInterface interface {
	...
	Apply(ctx context.Context, appProject *applicationv1alpha1.AppProjectApplyConfiguration, opts v1.ApplyOptions) (result *v1alpha1.AppProject, err error)
	AppProjectExpansion
}
...
// Apply takes the given apply declarative configuration, applies it and returns the applied appProject.
func (c *appProjects) Apply(ctx context.Context, appProject *applicationv1alpha1.AppProjectApplyConfiguration, opts v1.ApplyOptions) (result *v1alpha1.AppProject, err error) {
	if appProject == nil {
		return nil, fmt.Errorf("appProject provided to Apply must not be nil")
	}
	patchOpts := opts.ToPatchOptions()
	data, err := json.Marshal(appProject)
	if err != nil {
		return nil, err
	}
	name := appProject.Name
	if name == nil {
		return nil, fmt.Errorf("appProject.Name must be provided to Apply")
	}
	result = &v1alpha1.AppProject{}
